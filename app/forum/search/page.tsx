      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <Card className="p-8 text-center">
          ...
          <h1 className="text-2xl font-bold mb-2">
            {language === 'zh' ? '搜索论坛帖子' : 'Search Forum Threads'}
          </h1>
          <p className="text-muted-foreground mb-6">
            {language === 'zh'
              ? '输入关键词搜索相关帖子（至少2个字符）'
              : 'Enter keywords to search threads (at least 2 characters)'}
          </p>
    ...
    <div className="container mx-auto px-4 py-6">
          ...
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              ...
              {currentCategory && currentTag
                ? (language === 'zh'
                    ? `在 "${currentCategory.name}" 分类的 "#${currentTag.name}" 标签中搜索 "${searchQuery}"`
                    : `Searching for "${searchQuery}" in "${currentCategory.name_en || currentCategory.name}" category with "#${currentTag.name}" tag`)
                : currentCategory
                  ? (language === 'zh'
                      ? `在 "${currentCategory.name}" 分类中搜索 "${searchQuery}"`
                      : `Searching for "${searchQuery}" in "${currentCategory.name_en || currentCategory.name}" category`)
                  : currentTag
                    ? (language === 'zh'
                        ? `在 "#${currentTag.name}" 标签中搜索 "${searchQuery}"`
                        : `Searching for "${searchQuery}" with "#${currentTag.name}" tag`)
                    : (language === 'zh'
                        ? `搜索 "${searchQuery}" 的结果`
            ...
            {pagination && (
              <p className="mt-1 text-sm text-muted-foreground">
                {language === 'zh'
                  ? `找到 ${pagination.total} 个相关帖子`
                  : `Found ${pagination.total} thread${pagination.total !== 1 ? 's' : ''}`}
              </p>
            )}
          </div>
      ...
      {error && (
        <div className="mb-6 rounded-lg bg-red-50 p-4 text-red-800 dark:bg-red-950/50 dark:text-red-200">
          <p className="font-medium">{language === 'zh' ? '搜索失败' : 'Search failed'}</p>
            ...
            threads.length > 0 ? (
              ...
              <div className="text-center py-12 rounded-lg border border-dashed">
                ...
                <Button asChild>
                  <Link href="/forum/new">
                    <PlusCircle className="mr-2 h-4 w-4" />
                    {language === 'zh' ? '发布新帖' : 'Create new thread'}
                  </Link>
                </Button>
              </div>
            )
