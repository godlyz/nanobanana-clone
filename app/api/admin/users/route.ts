...
async function handleGET(req: NextRequest) {
  try {
    ...
    if (search) {
      ...
      // 🔥 老王修复：Supabase v2 .or()只接受单个字符串参数，用逗号分隔多个条件
    ...
    // 分页处理
    // 🔥 老王修复：Supabase v2中获取count需要单独调用，不能跟select混用
    ...
    return NextResponse.json({
      ...
      message: `获取到 ${formattedUsers.length} 个管理员用户`
    })
  ...
  try {
    ...
    if (!email || !name || !password) {
      return NextResponse.json({
        success: false,
        error: '缺少必需字段',
        message: 'email, name, password 是必需的'
      }, { status: 400 })
    }
    ...
    console.log(`✅ 管理员用户创建成功: ${email} (${role})`)
...
async function handlePUT(req: NextRequest) {
  try {
    ...
    if (updateError) {
      console.error('❌ 更新管理员用户失败:', updateError)
      return NextResponse.json({
        success: false,
        error: '更新管理员用户失败',
    ...
    console.log(`✅ 管理员用户更新成功: ${originalUser.email}`)
...
async function handlePATCH(req: NextRequest) {
  try {
    ...
    console.log(`✅ 管理员密码重置成功: ${user.email}`)
