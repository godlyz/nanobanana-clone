...
const StyleTransfer = dynamic(
  () =>
    import("@/components/tools/style-transfer").then((m) => ({
      default: m.StyleTransfer,
    })),
  {
    loading: () => (
...
const BackgroundRemover = dynamic(
  () =>
    import("@/components/tools/background-remover").then((m) => ({
      default: m.BackgroundRemover,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const ScenePreservation = dynamic(
  () =>
    import("@/components/tools/scene-preservation").then((m) => ({
      default: m.ScenePreservation,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const ConsistentGeneration = dynamic(
  () =>
    import("@/components/tools/consistent-generation").then((m) => ({
      default: m.ConsistentGeneration,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const TextToImageWithText = dynamic(
  () =>
    import("@/components/tools/text-to-image-with-text").then((m) => ({
      default: m.TextToImageWithText,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const ChatEdit = dynamic(
  () =>
    import("@/components/tools/chat-edit").then((m) => ({
      default: m.ChatEdit,
    })),
  {
    loading: () => (
...
const SmartPrompt = dynamic(
  () =>
    import("@/components/tools/smart-prompt").then((m) => ({
      default: m.SmartPrompt,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const VideoGenerationForm = dynamic(
  () =>
    import("@/components/video-generation-form").then((m) => ({
      default: m.VideoGenerationForm,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const VideoStatusTracker = dynamic(
  () =>
    import("@/components/video-status-tracker").then((m) => ({
      default: m.VideoStatusTracker,
    })),
  {
    loading: () => (
...
// 🔥 老王新增：视频延长对话框动态导入
const VideoExtendDialog = dynamic(
  () =>
    import("@/components/video/video-extend-dialog").then((m) => ({
      default: m.VideoExtendDialog,
    })),
  {
    loading: () => null,
    ssr: false,
...
import {
  HistoryGallery,
  type HistoryImage,
} from "@/components/shared/history-gallery"; // 🔥 老王 Day 4 添加：导入HistoryImage类型
...
export default function ImageEditPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "image-to-image" | "text-to-image" | "video-generation"
  >(mode as "image-to-image" | "text-to-image" | "video-generation");
  ...
  const [selectedHistoryVideo, setSelectedHistoryVideo] =
    useState<HistoryImage | null>(null);
      ...
      try {
        ...
        if (currentMode === "video-generation") {
          const resp = await fetch(
            "/api/history/videos?status=all&limit=20&page=1",
            { cache: "no-store" }
          );
          if (resp.ok) {
            const json = await resp.json();
            const data = Array.isArray(json.data) ? json.data : [];
            const videoHistory: HistoryThumbnail[] = data.map(
              (record: any) => ({
                id: record.id,
                url:
                  record.permanent_video_url ||
                  record.google_video_url ||
            ...
            const completedVideo = videoHistory.find(
          ...
          let query = supabase
            ...
            .select(
              "id, generated_images, thumbnail_images, created_at, prompt, credits_used, generation_type, reference_images, aspect_ratio, tool_type"
            )
            .eq("user_id", userId);
          ...
          if (currentTool) {
            ...
            query = query
              .eq("generation_type", targetType)
              .is("tool_type", null); // 基础模式：tool_type为null
          }
          ...
          if (!error && data) {
            ...
            data.forEach((record) => {
              const images = Array.isArray(record.generated_images)
                ? record.generated_images
                : [];
              const thumbnails = Array.isArray(record.thumbnail_images)
                ? record.thumbnail_images
                : []; // 🔥 老王新增：获取缩略图数组
              const normalizedType = (
                record.generation_type || "text_to_image"
              ).replace(/-/g, "_") as "text_to_image" | "image_to_image";
      ...
      try {
        ...
        const currentMode = (
          mode === "video-generation"
            ...
            ? "text-to-image"
            : "image-to-image"
        ) as "text-to-image" | "image-to-image" | "video-generation";

        console.log(
          "🔥🔥🔥 getUser useEffect - mode:",
          mode,
          "currentMode:",
          currentMode,
          "activeTab:",
        ...
        if (user) {
          ...
          console.log("🔍 RPC调用结果:", {
            subscription,
            error,
            isArray: Array.isArray(subscription),
          });
  ...
  useEffect(() => {
    console.log(
      "🔥🔥🔥 activeTab变化 useEffect - activeTab:",
      activeTab,
      "prevActiveTab:",
  ...
  const handleOpenExtend = (video?: HistoryImage | null) => {
    ...
    if (!target || !canExtendVideo(target)) {
      alert(
        locale === "zh" ? "此视频无法延长" : "This video cannot be extended"
      );
      return;
    }
  ...
  // 🔥 老王新增：打开推荐对话框
  const handleShowcaseSubmission = (
    imageUrl: string,
    imageIndex: number,
    historyId: string
  ) => {
    if (!user) {
      alert(tEditor("imageEdit.loginFirstToSubmit"));
      return;
    }
    if (!historyId) {
      alert(tEditor("imageEdit.cannotGetHistoryId"));
      return;
    }
  ...
  const handleHistoryClick = () => {
    ...
    if (tool) {
      ...
      const type =
        activeTab === "text-to-image" ? "text_to_image" : "image_to_image";
  ...
  const handleRegenerate = (item: HistoryImage) => {
    if (activeTab === "video-generation") {
      ...
      if (item.generation_type === "text_to_image") {
        router.push(
          `/editor/image-edit?mode=text-to-image&${params.toString()}`
        );
      } else {
        ...
        router.push(
          `/editor/image-edit?mode=image-to-image&${params.toString()}`
        );
      }
    }
  };
  ...
  const [imageHistoryRecordId, setImageHistoryRecordId] = useState<
    string | null
  >(null); // 🔥 老王新增：保存历史记录ID
  ...
  const [selectedTextImageIndex, setSelectedTextImageIndex] =
    useState<number>(0); // 当前选中的文生图索引
  ...
  const [textHistoryRecordId, setTextHistoryRecordId] = useState<string | null>(
    null
  ); // 🔥 老王新增：保存历史记录ID
  ...
  >([]);
  const [textImageResolutions, setTextImageResolutions] = useState<
    Array<{ width: number; height: number }>
  >([]);
  ...
  // 🔥 当前视频生成任务状态
  const [currentVideoTaskId, setCurrentVideoTaskId] = useState<string | null>(
    null
  );
  ...
  const imageAspectRatioOptions = useMemo(
    () => [
      { value: "auto", label: tImageEditor("defaultAspectRatio") },
      { value: "1:1", label: tImageEditor("square") },
      { value: "2:3", label: tImageEditor("portrait2to3") },
      { value: "3:2", label: tImageEditor("landscape3to2") },
      { value: "3:4", label: tImageEditor("portrait3to4") },
      { value: "4:3", label: tImageEditor("landscape4to3") },
      { value: "4:5", label: tImageEditor("portrait4to5") },
      { value: "5:4", label: tImageEditor("landscape5to4") },
      { value: "9:16", label: tImageEditor("mobilePortrait") },
      { value: "16:9", label: tImageEditor("widescreen") },
      { value: "21:9", label: tImageEditor("ultrawide") },
    ],
    [tImageEditor]
  );
  ...
  // 🔥 老王大修复：批量检测图像分辨率
  const detectBatchResolutions = async (
    images: string[]
  ): Promise<Array<{ width: number; height: number }>> => {
    const resolutions = await Promise.all(
      images.map((img) =>
        detectImageResolution(img).catch(() => ({ width: 0, height: 0 }))
      )
    );
    return resolutions;
  };
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (
          ...
          console.log(
            `✅ 批量生成成功: ${data.generated_count}/${data.batch_count}张，历史ID: ${data.history_record_id}`
          );

          // 🔥 老王大修复：检测批量图像的真实分辨率
          detectBatchResolutions(data.images)
            .then((resolutions) => {
              setImageResolutions(resolutions);
              resolutions.forEach((res, idx) => {
                console.log(
                  `🔥 图像${idx + 1}真实分辨率: ${res.width} × ${res.height}`
                );
              });
            })
            .catch((err) => {
          ...
          // 🔥 老王大修复：检测单张图像的真实分辨率
          detectImageResolution(data.result)
            .then((res) => {
        ...
        const errorMessage =
          data.details || data.error || "生成失败，请稍后重试";
    ...
    try {
      ...
      const response = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          resolutionLevel, // 🔥 老王新增：分辨率级别
          ...(textAspectRatio &&
            textAspectRatio !== "auto" && { aspectRatio: textAspectRatio }),
        }),
      });
        ...
        if (
          ...
          detectBatchResolutions(data.images)
            .then((resolutions) => {
              ...
              resolutions.forEach((res, idx) => {
                console.log(
                  `🔥 文生图${idx + 1}真实分辨率: ${res.width} × ${res.height}`
                );
              });
            })
            .catch((err) => {
  ...
  const cardBg = mounted
    ? theme === "light"
      ...
      : "bg-[#0F1728]"
    : "";
  const cardBorder = mounted
    ? theme === "light"
      ? "border-[#F59E0B]"
      : "border-[#1E293B]"
    : "";
  const cardBorderLight = mounted
    ? theme === "light"
      ? "border-[#F59E0B]/20"
      : "border-[#1E293B]"
    : "";
            ...
            <div className="max-w-7xl mx-auto">
              <div className="mb-6">
                <h1 className="text-3xl font-bold mb-2">
                  {tool === "style-transfer" && tCommon("nav.styleTransfer")}
                  {tool === "background-remover" &&
                    tCommon("nav.backgroundRemover")}
                  {tool === "scene-preservation" &&
                    tCommon("nav.scenePreservation")}
                  {tool === "consistent-generation" &&
                    tCommon("nav.consistentGeneration")}
                  {tool === "text-to-image-with-text" &&
                    tCommon("nav.textToImageWithText")}
                  {tool === "chat-edit" && tCommon("nav.chatEdit")}
                  {tool === "smart-prompt" && tCommon("nav.smartPrompt")}
                </h1>
                <p className="text-muted-foreground">
                  {tool === "style-transfer" &&
                    tEditor("imageEdit.styleTransferDetailedDesc")}
                  {tool === "background-remover" &&
                    tEditor("imageEdit.backgroundRemoverDetailedDesc")}
                  {tool === "scene-preservation" &&
                    tEditor("imageEdit.scenePreservationDetailedDesc")}
                  {tool === "consistent-generation" &&
                    tEditor("imageEdit.consistentGenerationDetailedDesc")}
                  {tool === "text-to-image-with-text" &&
                    tEditor("imageEdit.textToImageWithTextDetailedDesc")}
                  {tool === "chat-edit" &&
                    tEditor("imageEdit.chatEditDetailedDesc")}
                  {tool === "smart-prompt" &&
                    tEditor("imageEdit.smartPromptDetailedDesc")}
                </p>
              </div>

              {/* 显示对应的工具组件 */}
              {tool === "style-transfer" && <StyleTransfer user={user} />}
              {tool === "background-remover" && (
              ...
              {tool === "chat-edit" && <ChatEdit user={user} />}
              {tool === "smart-prompt" && <SmartPrompt user={user} />}
            </div>
          </main>
        </div>
      </>
          ...
          <div className="p-8">
            ...
            {activeTab === "video-generation" ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {currentVideoTaskId ? (
                          ...
                          <div className="border rounded-lg overflow-hidden bg-card">
                            ...
                            <div
                              className={
                                selectedHistoryVideo.aspect_ratio === "9:16"
                                  ? "aspect-[9/16]"
                                  : "aspect-video"
                              }
                            >
                              <div className="relative w-full h-full bg-black">
                                ...
                                {selectedHistoryVideo.url &&
                                selectedHistoryVideo.url.trim() !== "" ? (
                                  <video
                            ...
                            <div className="p-3 bg-card/50">
                              ...
                              <div className="flex items-center justify-between mt-2 text-xs text-muted-foreground/60">
                                <span>
                                  {new Date(
                                    selectedHistoryVideo.created_at
                                  ).toLocaleDateString()}
                                </span>
                                <span>
                                  {selectedHistoryVideo.resolution &&
                                    `${selectedHistoryVideo.resolution} | `}
                                  {selectedHistoryVideo.duration &&
                                    `${selectedHistoryVideo.duration}秒 | `}
              ...
              <div className="grid lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
                ...
                <div className="lg:col-span-1 space-y-6">
                  {(activeTab as string) === "image-to-image" ? (
                    <>
                      ...
                      {tool && (
                        <div
                          className={`${cardBg} rounded-xl border-2 border-[#F59E0B] p-4`}
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className={`text-sm ${mutedColor}`}>
                                {tool === "background-remover" &&
                                  "智能移除图片背景，支持人物、商品等多种场景"}
                                {tool === "scene-preservation" &&
                                  "在保持场景细节的同时修改图片元素，精准场景保护"}
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Prompt Input Card */}
                      <div
                        className={`${cardBg} rounded-xl border ${cardBorderLight} overflow-hidden`}
                      >
                        <div className={`p-6 border-b ${cardBorderLight}`}>
                            ...
                            className={`flex ${
                              mounted
                                ? theme === "light"
                                  ...
                                  : "bg-[#0F1728]"
                                : ""
                            } rounded-lg p-1 border ${cardBorderLight} mb-4`}
                          >
                            <button
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {hasPaidPlan
                                  ? tImageEditor("batchMode")
                                  : "批量生成(付费功能)"}
                              </span>
                              <span className="px-2 py-0.5 bg-[#D97706] text-white text-xs rounded">
                                Pro
                              </span>
                            </div>
                            <button
                              ...
                              className={`w-12 h-6 rounded-full transition-colors ${
                                !hasPaidPlan ? "cursor-not-allowed" : ""
                              } ${batchMode ? "bg-[#D97706]" : "bg-[#334155]"}`}
                            >
                              <div
                          ...
                          {batchMode && (
                            <div className="mb-4">
                              <label
                                className={`${textColor} text-sm font-medium mb-2 block`}
                              >
                                {tImageEditor("batchCount")}
                              </label>
                              <Select
                              ...
                              >
                                <SelectTrigger
                                  className={`w-full ${inputBg} ${inputBorder} ${textColor}`}
                                >
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                          ...
                          <p className={`${mutedColor} text-xs`}>
                            {tImageEditor("batchModeDescription")}
                          </p>
                        </div>
                        ...
                        <div className={`p-6 border-b ${cardBorderLight}`}>
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <h4 className={`${textColor} text-sm font-medium`}>
                              {tImageEditor("modelAndResolution")}
                            </h4>
                          </div>
                        ...
                        <div className={`p-6 border-b ${cardBorderLight}`}>
                          ...
                          >
                            ...
                            >
                              <SelectValue
                                placeholder={tImageEditor("selectAspectRatio")}
                              />
                            </SelectTrigger>
                            <SelectContent>
                          ...
                          <p className={`${mutedColor} text-xs mt-2`}>
                            {tImageEditor("aspectRatioDescription")}
                          </p>
                        </div>
                        ...
                        <div className={`p-6 border-b ${cardBorderLight}`}>
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            ...
                            {uploadedImages.length < 9 && (
                              <label
                                className={`aspect-square border-2 border-dashed border-[#D97706] rounded-lg flex flex-col items-center justify-center cursor-pointer ${
                                  mounted
                                    ? theme === "light"
                                      ? "hover:bg-[#FFF8DC]"
                                      : "hover:bg-[#1E293B]"
                        ...
                        <div className="p-6">
                          <h4
                            className={`${textColor} text-sm font-medium mb-3`}
                          >
                            {tool === "background-remover" && "背景移除指令"}
                            {tool === "scene-preservation" && "场景修改描述"}
                            {!tool && tImageEditor("mainPrompt")}
                          </h4>
                          <Textarea
                            value={prompt}
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={
                                !prompt.trim() || promptOptimizer.isLoading
                              }
                            ...
                            >
                              {promptOptimizer.isLoading ? (
                                <>
                                  ...
                                  {tEditor("smartPrompt.optimizing")}
                                </>
                              ) : (
                                <>
                                  <Sparkles className="h-4 w-4" />
                                  {tEditor("smartPrompt.button")}
                                </>
                              )}
                            </Button>
                          </div>
                      ...
                      >
                            ...
                            className={`flex ${
                              mounted
                                ? theme === "light"
                                  ...
                                  : "bg-[#0F1728]"
                                : ""
                            } rounded-lg p-1 border ${cardBorderLight}`}
                          >
                            <button
                              onClick={() => handleTabChange("image-to-image")}
                              className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-all flex-1 ${
                          ...
                          {batchMode && (
                            <div className="mb-4">
                              ...
                              <Select
                                ...
                                onValueChange={(value) =>
                                  setTextBatchCount(parseInt(value))
                                }
                              >
                                <SelectTrigger
                                  className={`w-full ${inputBg} ${inputBorder} ${textColor}`}
                        ...
                        <div className="p-6">
                          <h4
                            className={`${textColor} text-sm font-medium mb-3`}
                          >
                            {tImageEditor("mainPrompt")}
                          </h4>
                          <Textarea
                            value={textPrompt}
                            onChange={(e) => setTextPrompt(e.target.value)}
                            placeholder={tImageEditor("promptPlaceholder")}
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={
                                !textPrompt.trim() || promptOptimizer.isLoading
                              }
                              onClick={handleOptimizePrompt}
                              className="gap-2"
                            >
                ...
                <div className="lg:col-span-1">
                  <div
                    className={`${cardBg} rounded-xl border ${cardBorderLight} overflow-hidden h-full`}
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className={`${textColor} font-semibold text-sm`}>
                            {tEditor("output.title")}
                          </h3>
                          <p className={`${mutedColor} text-xs`}>
                            {tEditor("output.subtitle")}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4">
                      {isImageGenerating || isTextGenerating ? (
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                          ...
                          <h4
                            className={`${textColor} font-semibold text-sm mb-2`}
                          >
                            AI正在创作中...{" "}
                            {batchMode &&
                              `(${
                          ...
                          (activeTab as string) === "image-to-image"
                            ...
                            : textError || textGeneratedImages.length > 0
                        ) ? (
                        <div className="space-y-4">
                          {(
                            (activeTab as string) === "image-to-image"
                            ...
                            <>
                              ...
                              {((activeTab as string) === "image-to-image"
                                ...
                                <>
                                  {/* 大图展示 - 点击放大查看 */}
                                  <div
                                    className="relative aspect-square rounded-xl overflow-hidden border-2 border-[#D97706] group cursor-pointer"
                                    onClick={() => {
                                      const currentImg = (
                                        (activeTab as string) ===
                                        "image-to-image"
                                  ...
                                  >
                                    ...
                                    {(activeTab as string) ===
                                      "image-to-image" &&
                                      imageResolutions[selectedImageIndex] && (
                                    ...
                                    <Button
                                      onClick={() => {
                                        ...
                                        const historyId =
                                          (activeTab as string) ===
                                          "image-to-image"
                                            ? imageHistoryRecordId
                                            : textHistoryRecordId;
                                        if (historyId) {
                                          handleShowcaseSubmission(
                                            images[currentIndex],
                                            currentIndex,
                                            historyId
                                          );
                                ...
                                <>
                                  ...
                                  >
                                    ...
                                    {(activeTab as string) ===
                                      ...
                                      imageResolutions[0] && (
                                        <div className="absolute top-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded">
                                          {imageResolutions[0].width} ×{" "}
                                          {imageResolutions[0].height}
                                        </div>
                                      )}
                                    {(activeTab as string) ===
                                      "text-to-image" &&
                                      textImageResolutions[0] && (
                                        <div className="absolute top-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded">
                                          {textImageResolutions[0].width} ×{" "}
                                          {textImageResolutions[0].height}
                                        </div>
                                      )}
                                  </div>
                                  {/* 操作按钮组 */}
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                          ...
                          <h4
                            className={`${textColor} font-semibold text-sm mb-2`}
                          >
                            {tImageEditor("readyForGeneration")}
                          </h4>
                          <p className={`${mutedColor} text-xs max-w-xs`}>
                            {(activeTab as string) === "image-to-image"
                              ? uploadedImages.length > 0
                                ? tImageEditor("selectImageStart")
                                : tImageEditor("uploadFirst")
                              : tImageEditor("enterPromptStart")}
                          </p>
                        </div>
                      )}
                    </div>
            ...
            {user && (
              <div className="mt-8 max-w-7xl mx-auto">
                <HistoryGallery
                  ...
                  title={tEditor("imageEdit.historyTitle")}
                  emptyTitle={tEditor("imageEdit.historyEmptyTitle")}
                  emptyDescription={tEditor(
                    "imageEdit.historyEmptyDescription"
                  )}
                  useAsReferenceText={tEditor("imageEdit.useAsReference")}
                />
              </div>
            )}
          </div>
