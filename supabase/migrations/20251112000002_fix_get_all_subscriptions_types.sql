...
-- 🔥 老王修复：类型转换问题
-- 创建时间: 2025-11-12
-- 用途：修复 get_user_all_subscriptions 的类型匹配问题
-- =====================================================
...
BEGIN
    ...
    SELECT
        s.id,
        s.user_id,
        s.plan_tier::TEXT,              -- 🔥 类型转换
        s.billing_cycle::TEXT,          -- 🔥 类型转换
        s.monthly_credits,
        s.status::TEXT,                 -- 🔥 类型转换
        s.started_at,
        s.expires_at,
        s.creem_subscription_id::TEXT,  -- 🔥 类型转换
        -- 🔥 判断是否冻结
        CASE
            WHEN EXISTS (
...
-- 添加函数注释
COMMENT ON FUNCTION get_user_all_subscriptions IS '获取用户所有订阅（包括激活和冻结状态），修复类型转换问题';
