...
import { HistoryCarousel } from "@/components/shared/history-carousel"
...
export function StyleTransfer({ user }: StyleTransferProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.credits_used) {
        ...
        alert(t("styleTransfer.generationSuccess") || `生成成功！消耗了 ${data.credits_used} 积分`)
      }
  ...
  // 🔥 老王重构：下载历史记录图片
  const handleDownloadHistory = (imageUrl: string, recordId: string) => {
    const link = document.createElement('a')
    link.href = imageUrl
    link.download = `history-${recordId}.png`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  }
        ...
        >
          {isGenerating ? (
            ...
            <>
              ...
              {t("styleTransfer.generateNow")} ({language === 'zh' ? '消耗 2 积分' : 'Costs 2 Credits'})
            </>
          )}
        </Button>
